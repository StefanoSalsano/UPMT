...
unsigned int upmt_tg(struct sk_buff **pskb, const struct xt_action_param * tp) {
	...
	//printk("UPMT NETFILTER MODULE - upmt_tg - NEW CONNECTION -TGID %i [%s], nf_cnt %d, nf_info%d\n", skb->tgid, appname, skb->nfct->use.counter, skb->nfctinfo);
...
static int __init upmt_init(void) {
	...
	printk("upmt-conntracker xt_UPMT_ex - INIT COMPLETED\n");
	return 0;
...
static void __exit upmt_exit(void) {
	...
	printk("upmt-conntracker xt_UPMT_ex - EXIT COMPLETED\n");
}
