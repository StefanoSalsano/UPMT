...
public class UdpTransport implements Transport, UdpProviderListener
{
   ...
   {  if (udp_provider!=null)
      {  byte[] data=msg.toString().getBytes();
         ...
         /*
         //FIXME AFTER RESOLUTION OF TUNNEL KEEP ALIVE REMOVE THIS COMMENT 
          * start added code for keeap alive messages
         
         System.out.println("ZZZ__" + msg);
         System.out.println("ZZZ__" + dest_ipaddr + ":" + dest_port);
         System.out.println("ZZZ__" + udp_provider.getUdpSocket().getLocalPort() + " ---> " + dest_ipaddr.toString());
         TunnelManager tunnelManager = UPMTClient.instance().getTunnelManager();
         SipSignalManager sSM = UPMTClient.instance().getSignaler();
         String ifName = sSM.keepAlivePortToKASignalerInterface(udp_provider.getUdpSocket().getLocalPort());
         if(ifName != null)
        	 tunnelManager.assignSocketToTunnel("udp", udp_provider.getUdpSocket().getLocalPort(), dest_ipaddr.toString(), ConfigManager.instance().sbcSipPort, dest_ipaddr.toString(), ifName);
         //FIXME end*/
