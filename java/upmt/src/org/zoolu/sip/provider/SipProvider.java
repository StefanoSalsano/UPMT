...
public class SipProvider implements Configurable, TransportListener
{
   ...
   {  
	...
	{  init(via_addr,host_port);
	initLog();
	initSipTrasport(transport_protocols,null,ifaddr);
	}
	...
	{  init(via_addr,host_port);
	initLog();
	initSipTrasport(transport_protocols,transport_ports,ifaddr);
	}
	...
	//#else
	//      {  String filename = "/sdcard/upmt/" + SipStack.log_path + "/" + via_addr + "." + host_port;
	//#endif
   ...
   {  setOutboundProxy(null);
	...
	{  promiscuous_listeners.remove(listener);
	}
	}
   ...
   {  // in case it is passed only the 'user' field, add '@'<outbound_proxy>[':'<outbound_port>]
	...
	{  
		System.err.println(ExceptionPrinter.getStackTraceOf(e));
