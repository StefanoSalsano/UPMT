...
{      
	...
	//long stopTransaction;
	int delay;

	/** Number o retries in the current transaction */
	int numberRetry;
	...
	public void onReceivedMessage(SipProvider provider, Message msg)
	{
		...
		{  if (code<300) {
			//stopTransaction = System.currentTimeMillis();
			TimestampHeader tsh = msg.getTimestampHeader();
			if(tsh != null)
				delay = (int) (System.currentTimeMillis() - new Long(tsh.getValue()));
	...
	/** Return delay keep Alive */
	public int getDelay(){
		return delay;
	}

	/** Return numberRetry keep Alive */
