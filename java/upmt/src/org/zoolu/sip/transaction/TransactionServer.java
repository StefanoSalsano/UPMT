   ...
   {  if (msg.isRequest())
      {  if (statusIs(STATE_WAITING))
         ...
         {  // retransmission of the last response
            ...
            //Se il messaggio è un KeepAlive, in caso di ritrasmissione reimposto il timestamp
            if(msg.getTimestampHeader() != null)
            {
            	response.removeAllHeaders(SipHeaders.Timestamp);
            	response.addHeader(msg.getTimestampHeader(), false);
            }
