...
public abstract class BaseMessage extends org.zoolu.tools.MonitoredObject
{
   ...
   {	
	   Header h = getHeader(SipHeaders.Timestamp);
	   if (h==null) return null;
	      else return new TimestampHeader(h);
   }
   public void setTimestampHeader(String time)
   {	
	   Header h = getHeader(SipHeaders.Timestamp);
	   if (hasTimestampHeader()) removeAllHeaders(SipHeaders.Timestamp);
	      addHeader(new TimestampHeader(time), false);
   }
   
   public boolean hasTimestampHeader()
   {  return hasHeader(SipHeaders.Timestamp);
   }
