...
{  
   ...
   public static byte[] getTwoBytes(byte[] b, int offset) { return getBytes(b,offset,2); }
   
   /** Returns a 4-byte array from array <i>b</i> with offset <i>offset</i>. */
   public static byte[] getFourBytes(byte[] b, int offset) { return getBytes(b,offset,4); }
   ...
   public static void copyBytes(byte[] src, int src_off, byte[] dst, int dst_off, int len) { for (int k=0; k<len; k++) dst[dst_off+k]=src[src_off+k]; }
   ...
   public static byte[] ipv4addrStringToBytes(String addr)
   {  int begin=0, end;
      byte[] b=new byte[4];
      for (int i=0; i<4; i++)
      {  String num;
