...
public class SplitterLine extends OutputStream
{
   ...
   protected Object splitter_id;

   /** The output lines, as table of (Object)id-->(OutputStream)line. */
   protected Hashtable output_lines;
   ...
   {  this.splitter_id=splitter_id;
      ...
      if (DEBUG) System.err.println("SplitterLine-"+splitter_id+": new");
   }
   ...
   {  if (DEBUG) System.err.println("SplitterLine-"+splitter_id+": add: "+id+" "+os);
      output_lines.put(id,os);
   }

   /** Removes an output line. */
   public void removeLine(Object id)
   {  if (DEBUG) System.err.println("SplitterLine-"+splitter_id+": remove: "+id);
   ...
   {  /*for (Enumeration i=output_lines.elements(); i.hasMoreElements(); )
      {  ((OutputStream)i.nextElement()).write(b);
      }*/
      for (Enumeration i=output_lines.keys(); i.hasMoreElements(); )
      {  Object line_id=i.nextElement();
         try
         {  ((OutputStream)output_lines.get(line_id)).write(b);
         }
         catch (IOException e)
         {  if (DEBUG) System.err.println("SplitterLine-"+splitter_id+": ERROR while writing on line "+line_id);
            e.printStackTrace();
   ...
   {  return "SplitterLine-"+splitter_id;
   }
}
