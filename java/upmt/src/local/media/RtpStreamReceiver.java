...
public class RtpStreamReceiver extends Thread
{
   ...
   /** Constructs a RtpStreamReceiver.
     ...
     * @param socket the local receiver DatagramSocket
     * @listener the RtpStreamReceiver listener */
   public RtpStreamReceiver(OutputStream output_stream, DatagramSocket socket, RtpStreamReceiverListener listener)
   ...
   /** Inits the RtpStreamReceiver.
     ...
     * @param socket the local receiver DatagramSocket
     * @listener the RtpStreamReceiver listener */
   private void init(OutputStream output_stream, DatagramSocket socket, RtpStreamReceiverListener listener)
   ...
   {  if (rtp_socket!=null) return rtp_socket.getDatagramSocket().getPort();
      else return 0;
   }
   ...
   public void run()
   {
      ...
      if (DEBUG) println("RTP: localhost:"+rtp_socket.getDatagramSocket().getLocalPort()+" <-- remotesocket");
      ...
      {  rtp_socket.getDatagramSocket().setSoTimeout(SO_TIMEOUT);
         ...
         {  try
            {  // read a block of data from the rtp socket
               ...
               String addr=rtp_socket.getRemoteAddress().getHostAddress();
