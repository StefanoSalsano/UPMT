...
public class RtpStreamSender extends Thread
{
   ...
   {  if (remote_soaddr!=null && rtp_socket!=null)
      try
      {  rtp_socket=new RtpSocket(rtp_socket.getDatagramSocket(),InetAddress.getByName(remote_soaddr.getAddress().toString()),remote_soaddr.getPort());
      }
      catch (Exception e) {  e.printStackTrace();  }
   }
   ...
   public void run()
   {
      ...
      if (DEBUG) println("RTP: localhost:"+rtp_socket.getDatagramSocket().getLocalPort()+" --> "+rtp_socket.getRemoteAddress().toString()+":"+rtp_socket.getRemotePort());
