...
public class ContextManager {
	...
	private HashMap map;// for variables defined in files. include global and
						// normal.
	private List includeList;// store strings of include files which already
								// processed.
	private Stack backup;// for saving old namespaceOfCurrentFile
	private String namespaceOfCurrentFile;// absolute path of the current
											// processing file
	private String dest;// destination path
	private boolean export;// whether need to export
	private String comment = "//";// mark string for comment
	private String baseSrc; //absolute path of the base source dir or file
	private boolean isBaseSrcADirectory = false;
	private String baseDest; //absolute path of the base dest dir
	...
	public String getDefineValue(String key) {
		...
		if (map.containsKey(tmp))
			return (String) map.get(tmp);
		else if (map.containsKey("global." + key))
			return (String) map.get("global." + key);
		else
			return null;
	}
	...
	public String getDestPath(String sourcePath) {
		...
		//compares the sourcePath with the baseSrc directory
		//if the sourcePath is equal to the baseSrc directory
		if (sourcePath.equals(baseSrc) && !isBaseSrcADirectory) {
			return this.baseDest + sourcePath.substring(sourcePath.lastIndexOf(File.separator));
		}
		
//		//return this.dest;
//		if (sourcePath.startsWith(baseSrc)) {
			return this.baseDest  + sourcePath.substring(baseSrc.length());
//		} else {
//			return this.baseDest + sourcePath.substring(sourcePath.lastIndexOf(File.separator));
//		}
	...
	public void setBaseSrc(String absolutePath) {
		baseSrc = absolutePath;
	}

	public void setBaseDest(String absolutePath) {
		baseDest = absolutePath;
		
	}

	public void setBaseSrcADirectory(boolean isBaseSrcADirectory) {
		this.isBaseSrcADirectory = isBaseSrcADirectory;
	}
